...
async def verify_token(x_token: Annotated[str, Header()]):
    """Vérifie que le token passé dans les headers est valide"""
...
async def verify_key(x_key: Annotated[str, Header()]):
    """Vérifie que la clé API passée dans les headers est valide"""
    if x_key != API_KEY:
        raise HTTPException(
            status_code=403, 
...
@app.get("/events", dependencies=[Depends(verify_token), Depends(verify_key)])
...
@app.get("/events/by-day-of-week", dependencies=[Depends(verify_token), Depends(verify_key)])
...
@app.get("/events/search", dependencies=[Depends(verify_token), Depends(verify_key)])
...
@app.get("/events/hour", dependencies=[Depends(verify_token), Depends(verify_key)])
...
@app.get("/events/popular-days", dependencies=[Depends(verify_token), Depends(verify_key)])
...
@app.get("/events/place-stats", dependencies=[Depends(verify_token), Depends(verify_key)])
...
@app.get("/events/genres-trends", dependencies=[Depends(verify_token), Depends(verify_key)])
...
@app.get("/events/competition-impact", dependencies=[Depends(verify_token), Depends(verify_key)])
...
@app.get("/events/by-risk-category", dependencies=[Depends(verify_token), Depends(verify_key)])
...
@app.get("/events/by-followers", dependencies=[Depends(verify_token), Depends(verify_key)])
...
@app.get("/events/risk-by-followers", dependencies=[Depends(verify_token), Depends(verify_key)])
...
@app.get("/events/highest-risk", dependencies=[Depends(verify_token), Depends(verify_key)])
