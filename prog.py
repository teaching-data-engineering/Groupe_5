...
seen_artists = set()
page = 1
events_dict = {}  # Dictionnaire final pour le JSON
event_counter = 1  # Clé pour chaque événement
...
while True:
    ...
    if current_artists.issubset(seen_artists):
        break  # Tous doublons → fin de la boucle
...
with open("data_json/october_events.json", "w", encoding="utf-8") as f:
    json.dump(events_dict, f, ensure_ascii=False, indent=2)

print(f"{len(events_dict)} événements enregistrés dans 'october_events.json'.")
